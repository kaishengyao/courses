...
APACHE_ACCESS_LOG_PATTERN = '^(\S+) (\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] "(\S+) (\S+)\s*(\S*)\s*" (\d{3}) (\d+|\S+)'
...
uniqueHosts = hosts.map(lambda host : (host, 1)).reduceByKey(lambda a, b : a + b)
...
dayGroupedHosts = dayToHostPairTuple.map(lambda x : (x,1)).reduceByKey(lambda x,y:x+y)

dayHostCount = dayGroupedHosts.map(lambda ((d,h),c):((d,h),1)).reduceByKey(lambda x,y : x+y)

dailyHosts = (dayHostCount
              .map(lambda ((d,h),c) : (d, c))
...
groupedByDay = dayAndHostTuple.reduceByKey(lambda x,y:x+y).map(lambda ((d,h),c):(d,c)).reduceByKey(lambda x,y:x+y)

sortedByDay = groupedByDay.sortByKey()

avgDailyReqPerHost = (sortedByDay
                      .join(dailyHosts)
                      .map(lambda (d,(c,h)):(d,c/h))
                      .sortByKey().cache())
...
daysWithAvg = avgDailyReqPerHost.map(lambda (d,a):d).collect()
avgs = avgDailyReqPerHost.map(lambda (d,a):a).collect()
...
badUniqueEndpoints = badEndpoints.map(lambda end: (end,1)).reduceByKey(lambda x,y:x+y).map(lambda (e,c):e)
...
badEndpointsTop20 = badEndpointsSum.sortBy(lambda x:-x[1]).take(20)
...
errHostsTop25 = errHostsSum.sortBy(lambda x:-x[1]).take(25)
